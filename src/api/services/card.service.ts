...
import type { Card, CreateCardRequest, UpdateCardRequest, MoveCardRequest, AssignCardMemberRequest } from '../types/card';
...
export class CardService {
  ...
  static async getCardsByList(listId: string): Promise<Card[]> {
    const response = await $get<ApiResponse<Card[]>>(
      API_ENDPOINTS.CARDS.BY_LIST(listId)
    );
    return response.data.data;
  }
  ...
  static async createCard(data: CreateCardRequest): Promise<Card> {
    const response = await $post<ApiResponse<Card>>(
  ...
  static async updateCard(id: string, data: UpdateCardRequest): Promise<Card> {
    const response = await $put<ApiResponse<Card>>(
      API_ENDPOINTS.CARDS.UPDATE(id),
      data
    );
    return response.data.data;
  }
  ...
  static async assignMemberToCard(cardId: string, data: AssignCardMemberRequest): Promise<void> {
    await $post<ApiResponse<void>>(
      API_ENDPOINTS.CARDS.ASSIGN_MEMBER(cardId),
      data
    );
  }
  ...
  static async removeMemberFromCard(cardId: string, userId: string): Promise<void> {
    await $delete<ApiResponse<void>>(
      API_ENDPOINTS.CARDS.REMOVE_MEMBER(cardId, userId)
    );
  }
  ...
  static async getCardMiscComments(cardId: string): Promise<MiscComment[]> {
    const response = await $get<ApiResponse<MiscComment[]>>(
  ...
  static async getCardActivities(cardId: string): Promise<Activity[]> {
    const response = await $get<ApiResponse<Activity[]>>(
      API_ENDPOINTS.ACTIVITIES.BY_CARD(cardId)
    );
    return response.data.data;
  }
  ...
  static async deleteCardAttachment(cardId: string, attachmentId: string): Promise<void> {
    await $delete<ApiResponse<void>>(
      `${API_ENDPOINTS.CARDS.ATTACHMENTS(cardId)}/${attachmentId}`
    );
  }
