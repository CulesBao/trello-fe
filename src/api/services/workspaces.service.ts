...
import type { Workspace, CreateWorkspaceRequest, UpdateWorkspaceRequest, AddWorkspaceMemberRequest } from '../types/workspace';
...
export class WorkspaceService {
  ...
  static async getWorkspaceById(id: string): Promise<Workspace> {
    const response = await $get<ApiResponse<Workspace>>(
  ...
  static async createWorkspace(data: CreateWorkspaceRequest): Promise<Workspace> {
    const response = await $post<ApiResponse<Workspace>>(
  ...
  static async updateWorkspace(id: string, data: UpdateWorkspaceRequest): Promise<Workspace> {
    const response = await $put<ApiResponse<Workspace>>(
   ...
   */
  static async addWorkspaceMember(workspaceId: string, data: AddWorkspaceMemberRequest): Promise<void> {
  ...
  static async getWorkspaceBoards(workspaceId: string): Promise<Board[]> {
    const response = await $get<ApiResponse<Board[]>>(
      API_ENDPOINTS.WORKSPACES.BOARDS(workspaceId)
    );
    return response.data.data;
  }
}
